...
- Direct pushes to `main` are the normal workflow here (no PR review step) — always confirm with the user before `git push`, and write commit messages that explain *why*, not just what changed (this repo's history leans on that for context later).
